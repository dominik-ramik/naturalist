import { cssColorNames, indexOfCaseInsensitive, isValidHttpUrl, pad } from "../components/Utils.js";
...
export let DataManager = function () {
        ...
        function compileDataMeta(lang) {
            ...
            data.common.getAllColumnInfos(lang.code).forEach(function (info) {
                ...
                matchingComputedDataPaths.forEach(function (computedDataPath) {
                    ...
                    let dataType = "custom";
                    if (info.table == data.sheets.content.tables.customDataDefinition.name) dataType = "custom";
                    if (info.table == data.sheets.content.tables.media.name) dataType = "media";
                    if (info.table == data.sheets.content.tables.maps.name) dataType = "map";
                    if (info.table == data.sheets.content.tables.accompanyingText.name) dataType = "text";
    ...
    function loadData(table) {
                    ...
                    if (info.role == "taxon") {
                        ...
                        if (taxonIsEmpty) {
                            ...
                            if (!doneWithTaxa) {
                                ...
                                log("error", _tf("dm_incomplete_taxa_info_row", [(rowIndex + data.common.checklistHeadersStartRow), info.name]));
                            }
                        }

                    } else if (info.role == "data") {
                        includeTreefiedData(rowObj.d, headers, row, dataPath.modify.pathToSegments(info.name), 0, info, "", lang.code);
                    } else {
        ...
        function includeTreefiedData(rowObjData, headers, row, pathSegments, pathPosition, info, computedPath, langCode) {
            ...
            if (currentSegment == "#") {
                ...
                if (pathPosition == pathSegments.length - 1) {
                    ...
                    let genericData = getGenericData(headers, row, computedPath, info, langCode);
                    //if (genericData) {
                ...
                case "general":
                    ...
                    if (stringValue == null) {
                        ...
                        if (valueRegex.test(template)) {
                            log("warning", _tf("dm_defined_column_not_present", [computedPath, info.table, data.sheets.content.name]));
                        }
                        ...
                        switch (expectedType) {
                            ...
                            case "number":
                                ...
                                if (Number.isNaN(number)) {
                                    log("error", _tf("dm_value_not_number", [stringValue, computedPath]));
                                }
                                return number;
                                break;
                            case "date":
        ...
        function readMedia(headers, row, path, langCode) {
            ...
            for (let index = 1; index <= 50; index++) {
                singleMedia = readSingleMedia(headers, row, path + index.toString(), langCode);
        ...
        function readSingleMedia(headers, row, path, langCode) {
            if (headers.indexOf(path) < 0 && headers.indexOf(path + ":" + langCode) < 0 && headers.indexOf(path + ".source") < 0 && headers.indexOf(path + ".source:" + langCode) < 0) {
                return null;
            }
            ...
            if (source === null || (source !== null && title === null)) {
                log("error", _tf("dm_media_column_names", [path, path, (path + ".source"), (path + ".title")]));
            }
        ...
        function readTaxon(headers, row, path, langCode) {
            ...
            if (name === null || (name !== null && authority === null)) {
                log("error", _tf("dn_taxon_column_names", [path, path, (path + ".name"), (path + ".authority")]));
            }
    ...
    function checkMetaValidity() {
        ...
        data.common.getAllColumnInfos(data.common.languages.defaultLanguageCode).forEach(function (item) {
            if (Object.keys(uniqueColumnNames).indexOf(item.name.toLowerCase()) < 0) {
                ...
                log("error", _tf("dm_column_name_duplicate", [item.name, item.table, uniqueColumnNames[item.name].table]));
            }
        });
        ...
        data.common.languages.supportedLanguages.forEach(function (lang) {
            ...
            if (isNaN(hue) || hue < 0 || hue > 360) {
                log("error", _tf("dm_hue_value", [data.sheets.appearance.tables.customization.name]));
            }
        });
    ...
    function postprocessMetadata() {
        ...
        if (customChecklistDataStartRow && !isNaN(parseInt(customChecklistDataStartRow))) {
            data.common.checklistHeadersStartRow = parseInt(customChecklistDataStartRow);
        }
