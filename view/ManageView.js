...
export let ManageView = {
  ...
  logList: function () {
    ...
    const deduplicatedMessages = dataman.loggedMessages.filter(
      (item, index) =>
        dataman.loggedMessages.findIndex((find) => find.message == item.message) === index
    );
  ...
  uploadError: function () {
    return [
      m("div", [
        ...
        ManageView.stateMessageCode?.length > 0 &&
        ManageView.stateMessageCode != "other_upload_error"
  ...
  uploadOrDownloadData: function () {
    return [
      ...
      m(".static-download", [
        ...
        m(
          ".manage-message",
          m.trust(marked.parse(_t("download_for_manual_update")))
        ),
        m(
          "button.uploadbutton",
          {
            onclick: function (e) {
              var blob = new Blob(
                [
                  compressor.compress(
                    JSON.stringify(dataman.getCompiledChecklist())
                  ),
                ],
              ...
              if (request.readyState === 4) {
                if (request.status === 200) {
                  ...
                  try {
                    ...
                    if (
                      request.responseText.indexOf("POST Content-Length") >=
                        0 &&
                    ...
                    result = {
                      ...
                      details: [
                        ...
                        m(
                          ...
                          _t("server_returned_odd_message_details") + ex
                        ),
                      ],
                    };
                  }
                  ...
                  if (request.statusText.toLowerCase() == "not found") {
                    ...
                    ManageView.stateDetails =
                      _t("network_error") + " " + request.statusText;
                  }
      ...
      Checklist._isDataReady
        ...
        : m(".no-data", [
            m(
              ...
              m(
                "img.upload-icon[src=img/icon_maskable.svg][style=width: auto; height: 15em; margin: 0px]"
              ),
              m("h1", _t("fresh_install_welcome")),
              m("div", _t("fresh_install_welcome_message")),
              m(
                ".manage-message",
                m.trust(marked.parse(_t("starting_from_scratch")))
              ),
