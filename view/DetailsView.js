import { TabsContainer, TabsContainerTab } from "../components/TabsContainer.js";
...
export let DetailsView = {
    ...
    view: function(vnode) {
        ...
        DetailsView.taxonName = m.route.param("taxon") ? m.route.param("taxon") : "";
        ...
        return m(".details", [
            ...
            m(TabsContainer, { tabs: TabsForDetails(Checklist.getDetailsTabsForTaxon(DetailsView.taxonName), taxon, DetailsView.taxonName), activeTab: tab }),
        ]);
    }
}
...
function taxonomyCrumbs(taxonName) {
    ...
    return taxon.t.map(function(taxonName, index) {
        ...
        return m(".details-taxon-crumb", [
            m(".crumb-taxon-level", Checklist.getTaxaMeta()[Object.keys(Checklist.getTaxaMeta())[index]].name),
...
function TabsForDetails(detailsTabs, taxon, taxonName) {
    ...
    Object.keys(detailsTabs).forEach(function(key) {
        ...
        let onClickCallback = function() {
            ...
            routeTo('/details/' + taxon.t[taxon.t.length - 1].n + '/' + Settings.currentDetailsTab());
        }
...
function TabMedia(tabData, taxon, taxonName) {
        ...
        if (meta.datatype == "media") {
            if (tabData.indexOf(metaKey) >= 0) {
                ...
                mediaItems.forEach(function(mediaItem) {
                    if (mediaItem == undefined || mediaItem == null || !mediaItem.source || mediaItem.source == "") {
                        return;
                    }
                    ...
                    if (!mediaRenderingData[mediaType].hasOwnProperty(metaKey)) {
                        mediaRenderingData[mediaType][metaKey] = { title: mediaTitle, mediaList: [] };
                    }
    ...
    return m(".media-list", Object.keys(mediaRenderingData).map(function(dataKey) {
        return m(".media-type-" + dataKey, Object.keys(mediaRenderingData[dataKey]).map(function(mediaTitleKey) {
            let mediaSet = mediaRenderingData[dataKey][mediaTitleKey];

            return m(".media-set", [
                m(".media-set-title", mediaSet.title),
                m(".media-set-list", mediaSet.mediaList.map(function(media, index) {
                    switch (dataKey) {
                        case "image":
                            return m(".media-image", [
                                m(".image-wrap", {
                                    onclick: function(e) {
                                        ...
                                        this.getElementsByTagName("img")[0].classList.toggle("clickable");
                                        e.preventDefault();
                                        e.stopPropagation();
                                    }
...
function TabMap(tabData, taxon, taxonName) {
    ...
    Object.keys(Checklist.getDataMeta()).forEach(function(metaKey) {
        ...
        if (meta.datatype == "map") {
            ...
            if (!mapsRenderingData[mapType].hasOwnProperty(metaKey)) {
                mapsRenderingData[mapType][metaKey] = { title: mapTitle, mediaList: [] };
            }
    ...
    return m(".media-list", Object.keys(mapsRenderingData).map(function(dataKey) {
        return m(".media-type-" + dataKey, Object.keys(mapsRenderingData[dataKey]).map(function(mediaTitleKey) {
            let mediaSet = mapsRenderingData[dataKey][mediaTitleKey];

            return m(".media-map-set", [
                m(".media-set-list", mediaSet.mediaList.map(function(media) {
                    switch (dataKey) {
                        case "image":
                            return m(".media-image", [
                                m(".image-wrap", {
                        ...
                        case "regions":
                            ...
                            [].concat(Checklist.getMapRegionsMeta(true), Checklist.getMapRegionsMeta()).forEach(function(mapRegionMeta) {
                                mapRegionsSplit.forEach(function(individualRegion) {
                                    if (getSuffixOfRegion(individualRegion).suffix == mapRegionMeta.suffix && presentRegionsMetaSuffixes.indexOf(mapRegionMeta.suffix) < 0) {
                            ...
                            function colorMap(objectElement) {
                                ...
                                if (objectElement.hasAttribute("data-usedregions")) {
                                    objectElement.getAttribute("data-usedregions").split(" ").forEach(function(suffixlessRegionCode) {
                                        let regionElements = objectElement.contentDocument.getElementsByClassName(suffixlessRegionCode);
                                ...
                                if (media.regions) {
                                    ...
                                    media.regions.split(" ").forEach(function(region) {
                                        ...
                                        let suffixlessRegionCode = region.substring(0, region.length - mapRegionMeta.suffix.length);
                            ...
                            return m(".media-map", [
                                ...
                                }, m("object#map_" + media.metaKey + "[style=pointer-events: none;][type=image/svg+xml][data=usercontent/maps/" + media.url + "]", {
                                    onload: function() {
                                        colorMap(this);
                                    },
                                })),
                                media.regions ? m(".legend", Object.keys(presentRegionsMeta).length == 0 ? null : Object.values(presentRegionsMeta).map(function(regionMeta) {
...
function TabText(tabData, taxon, taxonName) {
    ...
    Object.keys(Checklist.getDataMeta()).forEach(function(metaKey) {
        ...
        if (meta.datatype == "text") {
            if (tabData.indexOf(metaKey) >= 0 && taxon.d[metaKey].length > 0) {
                ...
                mdIndex += "- <div class='index-head' onclick=\"document.getElementById('" + metaKey + "').scrollIntoView({behavior: 'smooth'}, true)\">" + meta.title + "</div>\n";
    ...
    htmlText = (mdIndex.length > 0 && tabData.length > 1 ? marked.parse(mdIndex) + "\n\n" : "") + htmlText;

    let routeUri = window.location.origin + window.location.pathname + "#!/view/";

    htmlText = htmlText.replace(/<img /gi, "<img onclick=\"window.location.href = '" + routeUri + "' + this.src;\" ");
...
function TabExternalSearch(tabData, taxon, taxonName) {
    ...
    return m(".search-engines", tabData.map(function(engine) {

        let translatedURL = resolveTemplate(engine.url, "");

        return m(".search-engine", {
            onclick: function() {
                window.open(translatedURL, "_blank");
            }
        }, [
            m("img.engine-icon[src=usercontent/online_search_icons/" + engine.icon + "]"),
...
function resolveTemplate(template, currentValue) {
    ...
    let data = Checklist.getDataObjectForHandlebars(currentValue, DetailsView.taxonData, DetailsView.taxonName, DetailsView.taxonAuthority);
