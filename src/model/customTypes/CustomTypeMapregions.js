...
export let customTypeMapregions = {
  ...
  meta: {
    summary: "Geographic distribution data encoded as region code / status / note tuples. Rendered as a coloured SVG map in the Details pane and as an inline text list of region names in the taxon card. The most sophisticated built-in data type.",
    whenToUse: "Any geographic distribution data where you want an interactive choropleth map in the Details pane and/or an inline region list in the taxon card.",
    behaviorFulltextIndexing: "Region names (resolved from the Map Regions Information table) are indexed for full-text search.",
    detailsPaneTab: "Map",
    inputFormats: [
      {
        label: "Format 1: Inline single cell",
        syntax: "All regions encoded in one cell, separated by `|`. Each region uses `RegionCode:Status#Note1#Note2`. Status and notes are optional. Escape a literal `#` as `\\#`.",
        example: {
          columns: ["distribution"],
          rows: [
            ["fr:native#Verified 2022 | de:introduced | gb"],
            ["fr | de:native#Data from @smith2020 | at:extirpated"],
          ],
        },
      },
      {
        label: "Format 2: Per-column",
        syntax: "Each region has its own column named `<columnname>.<regioncode>`. Cell content is `Status#Note`.",
        example: {
          columns: ["map.fr", "map.de", "map.gb"],
          rows: [["native", "introduced#2015", ""], ["native", "", "native"]],
        },
      },
    ],
    notes: [
      {
        type: "tip",
        text: "The **Template** column must specify the SVG file path for the Details pane map to appear — e.g. `maps/world.svg`. Without a template, only the inline region name list is rendered. See [Data Types → mapregions](/reference/data-types#mapregions) for SVG authoring requirements.",
      },
      {
        type: "tip",
