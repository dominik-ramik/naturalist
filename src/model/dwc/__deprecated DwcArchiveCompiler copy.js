...
// EML fetch helper
//
// Fetches an eml.xml file from the usercontent/ folder by relative path.
// Returns the file content as a string, or null if the fetch fails for any
// reason (path traversal attempt, HTTP error, SPA soft-404, network error).
// ═══════════════════════════════════════════════════════════════════════════════
...
function renderTemplate(template, resolveCol) {
  ...
  for (const seg of template.split("|")) {
    ...
    // A segment is included only if every placeholder it contains resolves to a
    // non-empty value. Missing any placeholder drops the whole segment, but does
    // not affect the other segments.
...
export async function compileDwcArchive(params) {
  ...
  // Step 1 - Parse rows
  //
  // eml:fromFile: rows → rawEmlFileMappings (file-based EML, per archive)
  // eml: field rows    → rawEmlMappings (constructed EML, per archive)
  // all other rows     → parsedMappings (for CSV)
  //
  // Both eml: row types carry an exportTo set so EML can be resolved
  // independently per archive in Step 8.
  // ───────────────────────────────────────────────────────────────────────────
    ...
    if (termRaw.toLowerCase().startsWith("eml:")) {
      ...
      if (emlTermRest.toLowerCase().startsWith("fromfile:")) {
        ...
        const exportToSet = exportToRaw === "" || exportToRaw === "all"
          ? new Set(["checklist", "occurrences"])
          : new Set(exportToRaw.split(",").map(s => s.trim()).filter(Boolean));
        rawEmlMappings.push({ path: emlTermRest, directive, constantValue: cv, exportTo: exportToSet });
      }
    } else {
      const exportToRaw = (row.exportTo || "").toString().trim().toLowerCase();
      const exportToSet = exportToRaw === "" || exportToRaw === "all"
  ...
  // Validate eml: field rows per archive: each archive that has eml: rows
  // must include at least one creator name field.
  for (const archiveName of ["checklist", "occurrences"]) {
    const archiveEmlRows = rawEmlMappings.filter(em => em.exportTo.has(archiveName));
    if (archiveEmlRows.length === 0) continue;
    const emlPathsLo = new Set(archiveEmlRows.map(em => em.path.toLowerCase()));
    ...
    if (!hasCreatorName) {
      Logger.error(
        ...
        `for the <b>${archiveName}</b> archive. ` +
    ...
    if (!emlPathsLo.has("creator.email")) {
      Logger.warning(
        ...
        `(missing for the <b>${archiveName}</b> archive).`,
        "DwC Archive"
      );
    }
  }
  ...
  // Warn if multiple eml:fromFile rows target the same archive - only the first will be used.
  for (const archiveName of ["checklist", "occurrences"]) {
    const matching = rawEmlFileMappings.filter(ef => ef.exportTo.has(archiveName) && ef.path);
    if (matching.length > 1) {
      Logger.warning(
        `DwC Archive: Multiple <b>eml:fromFile</b> rows target the <b>${archiveName}</b> archive. ` +
        `Only "<em>${matching[0].path}</em>" will be used; the rest are ignored.`,
        "DwC Archive eml.xml"
      );
    }
  }
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // ───────────────────────────────────────────────────────────────────────────
  // Step 8 - EML resolution (per archive)
  //
  // resolveEmlForArchive() is called independently for each enabled archive
  // at ZIP time (Step 10).  It tries, in order:
  //   1. An eml:fromFile row whose exportTo covers this archive → fetch;
  //      Logger.error and return null if the file cannot be retrieved.
  //   2. eml: field rows whose exportTo covers this archive → build minimal EML.
  //   3. Neither configured → Logger.warning, return null (archive produced
  //      without eml.xml; GBIF will be unhappy).
  // ───────────────────────────────────────────────────────────────────────────

  const resolveEmlForArchive = async (archiveName) => {
    // ── 1. eml:fromFile ──────────────────────────────────────────────────────
    const fileMapping = rawEmlFileMappings.find(ef => ef.exportTo.has(archiveName) && ef.path);
    if (fileMapping) {
      const fetched = await tryFetchEml(fileMapping.path);
      if (fetched) {
        Logger.info(
          `DwC Archive: Using <em>eml:fromFile</em> "<em>${fileMapping.path}</em>" ` +
          `for the <b>${archiveName}</b> archive.`,
          "DwC Archive eml.xml"
        );
        return fetched;
      ...
      Logger.error(
        `DwC Archive: <b>eml:fromFile:${fileMapping.path}</b> could not be fetched from ` +
        `usercontent/ for the <b>${archiveName}</b> archive. ` +
    ...
    // ── 2. eml: field rows ───────────────────────────────────────────────────
    const archiveEmlRows = rawEmlMappings.filter(em => em.exportTo.has(archiveName));
    if (archiveEmlRows.length > 0) {
      const emlFieldValues = {};
      for (const em of archiveEmlRows) {
        let val = "";
      ...
      const emlTitle = emlFieldValues["title"] || "Untitled dataset";
      const emlAbstract = stripMarkdown(emlFieldValues["abstract"] || "");
    ...
    // ── 3. No EML configured for this archive ────────────────────────────────
    Logger.warning(
      `DwC Archive: No EML metadata configured for the <b>${archiveName}</b> archive. ` +
      `Add <em>eml:fromFile</em> or <em>eml:</em> field rows targeting this archive. ` +
