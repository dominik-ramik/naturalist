...
function getNextRowIndex() {
  return rowCounter.value++
}
// Reset when Sheet unmounts/remounts (v-if case)
onMounted(() => { rowCounter.value = 0 })
...
function getStickyLeft(colIndex) {
  ...
  for (let i = 0; i < colIndex; i++) {
    ...
    if (!def)            { px += DEF_COL_W; continue }
    if (def.empty)       { px += EMPTY_COL_W; continue }
    const w = def.width
    if (w && w.endsWith('px'))  { px += parseFloat(w); continue }
    if (w && w.endsWith('rem')) { px += parseFloat(w) * 16; continue }
    px += DEF_COL_W
  }
  return px + 'px'
}
...
provide('ss:freezeRows',      computed(() => props.freezeRows))
provide('ss:freezeCols',      computed(() => props.freezeCols))
provide('ss:striped',         computed(() => props.striped))
...
const headerCols = computed(() => {
  if (colDefs.value.length > 0) {
    ...
    return colDefs.value.map(def => ({
      empty: def.empty,
      label: def.empty ? '' : colLetter(letterIdx++),
      width: def.empty ? EMPTY_COL_W + 'px'
                       : (def.width ?? DEF_COL_W + 'px'),
    }))
  }
  ...
  <div class="ss-sheet" v-show="isActive">
    <div class="ss-scroll-container" :style="scrollStyle">
      <table class="ss-table" ref="tableEl" style="margin: 0px">
        ...
        <colgroup>
          <col :style="{ width: ROW_NUM_W + 'px', minWidth: ROW_NUM_W + 'px' }" />
          <col
        ...
        <thead>
          <tr class="ss-col-headers">
            ...
            <th
              v-for="(col, i) in headerCols"
              :key="i"
              :class="['ss-col-header-cell', col.empty && 'ss-col-header-empty']"
            >
              {{ col.label }}
            </th>
          </tr>
