...
export let ExcelBridge = function (excelFile) {
      ...
      if (sheet.type == "meta") {
        Object.keys(sheet.tables).forEach(function (tableKey) {
          ...
          data.common.languages.supportedLanguages.forEach(function (lang) {
            table.data[lang.code] = subTableToMultilingualObject(
              workbook,
              sheet.name,
              table,
              lang
  ...
  function loadKnownLanguages(workbook) {
    ...
    for (let row = 1; row < languageTable.length; row++) {
      ...
      let langCode = getCellFromSubTable(
        languageTable,
        row,
        nCode,
        "",
      ...
      let langName = getCellFromSubTable(
        languageTable,
        row,
        nName,
        "",
      ...
      let fallbackLang = getCellFromSubTable(
        languageTable,
        row,
        nFallback,
        "",
      ...
      data.common.languages.supportedLanguages.push({
        code: langCode,
        name: langName,
  ...
  function getMultilingualBestFitColumnIndex(
    headers,
    columnName,
    language,
    subTableName
  ...
  function getCellFromSubTable(
    ...
    let colIndex = getMultilingualBestFitColumnIndex(
      subTable[0],
      columnName,
      language,
      subTableName
  ...
  function subTableToMultilingualObject(workbook, sheetName, tableInfo, lang) {
    ...
    for (let row = 1; row < rawSubTable.length; row++) {
      ...
      Object.keys(tableInfo.columns).forEach(function (columnKey) {
        lineObject[columnKey] = getCellFromSubTable(
          rawSubTable,
          row,
          tableInfo.columns[columnKey].name,
          lang.code,
          tableInfo.name
        );
      });
      ...
      for (const header of headers) {
        ...
        if (
          ...
          indexOfCaseInsensitive(
            headers,
            header + ":" + data.common.languages.defaultLanguageCode
          ) >= 0
        ) {
          throw (
            "You have both '" +
            ...
            "' in table '" +
            tableName +
            "' - to prevent ambiguity, keep only the '" +
            header +
            "' column"
          );
        }
        ...
        if (header.split(":").length > 2) {
          throw (
            ...
            "'  in table '" +
            tableName +
            "' is malformed - only one symbol ':' is allowed, which separates the column name from the language code"
          );
        }
      }

      const expectedHeaders = Object.values(tableInfo.columns).map(
        ...
        Object.keys(tableInfo.columns).forEach(function (columnKey) {
          ...
          if (!columnMeta.integrity.supportsMultilingual) {
            ...
            subTable[0].forEach(function (header) {
              if (
                header.toLowerCase() ==
                columnMeta.name.toLowerCase() + ":" + lang?.code.toLowerCase()
              ) {
                multilingualColumns.push(
                  header.substring(columnMeta.name.length)
                );
              }
            });
